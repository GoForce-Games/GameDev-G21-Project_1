...
bool Render::LoadState(pugi::xml_node node) {
	...
	camDebugOffset.x = node.child("camera").attribute("x").as_int();
	camDebugOffset.y = node.child("camera").attribute("y").as_int();
...
bool Render::SaveState(pugi::xml_node node) {
	...
	camNode.append_attribute("x").set_value(camDebugOffset.x);
	camNode.append_attribute("y").set_value(camDebugOffset.y);
